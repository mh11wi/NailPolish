...
export default {
  ...
  methods: {
     ...
     */
    getImage(id) {
      return '@/assets/images/nailart/' + id + process.env.VUE_APP_EXTENSION;
    },
    ...
    getPolishImage(id) {
      if (this.polishes[id - 1].type == 'Topper') {
        const image = '@/assets/images/polishes/' + process.env.VUE_APP_DEFAULT_BASE + '/' + id + process.env.VUE_APP_EXTENSION;
